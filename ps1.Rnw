...
to ensure that I pull out the correct line. Then, I have to sort the
data by column 6 which represents value. I found that there are \textbf{``}
in each column which make me hard to delimit column by \textbf{,}.
Hence, I used \textbf{sed} to eliminate those \textbf{``} and then
sort the data from big to small \textbf{(-r)} by column 6 by the command
\textbf{-k6} with \textbf{-t','} indicating that I want to use , to
...
function dldata() { 	
wget "http://data.un.org/Handlers/DownloadHandler.ashx?DataFilter=itemCode:$1&DataMartId=FAO&Format=csv&c=2,3,4,5,6,7&s=countryName:asc,elementCode:asc,year:desc" -O $1.zip unzip -p $1.zip }
